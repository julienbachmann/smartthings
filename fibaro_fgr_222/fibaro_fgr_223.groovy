metadata {
    definition (name: "Fibaro FGR-223", namespace: "julienbachmann", author: "Julien Bachmann") {
        capability "Sensor"
        capability "Actuator"
...
def configure() {
    ...
    delayBetween([
        zwave.configurationV1.configurationSet(parameterNumber: 150, size: 1, scaledConfigurationValue: 2).format(),  // start calibration
        zwave.switchMultilevelV1.switchMultilevelGet().format(),
        zwave.meterV2.meterGet(scale: 0).format(),
        zwave.meterV2.meterGet(scale: 2).format(),
], 500)
...
private getParamsMd() {
    return [        
    [id:  151, size: 1, type: "number", range: "1..6", defaultValue: 0, required: false, readonly: false,
        name: "Roller Shutter operating modes",
        description: "1 - Roller Blind Mode, with positioning\n" +
